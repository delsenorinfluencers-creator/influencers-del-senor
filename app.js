...
const programs=[
["un-cafe-con-aroma-de-fe","Un café con aroma de fe","Conversaciones, encuentros y reflexiones desde la fe."],
["el-podcast","El Podcast","Conversaciones para escuchar, pensar y vivir el Evangelio."],
["el-youcat-te-conecta","El Youcat te conecta","Formación juvenil y Catecismo."],
["la-voz-lider","La voz líder","Historias, liderazgo y servicio."],
["hagan-lo-que-el-les-diga","Hagan lo que Él les diga","Reflexiones desde la Palabra y María."],
["una-palabra-en-60-segundos","Una palabra en 60 segundos","Un mensaje breve para la vida diaria."],
["fe-y-vida-diaria","Fe y vida diaria","La fe aplicada a los desafíos de cada día."],
["la-santa-misa","La Santa Misa","Contenido para acompañar y profundizar en la Eucaristía."],
["kerigma-en-fuego","Kerigma en fuego","Anuncio de Jesucristo vivo."],
["semilla-vocacional","Semilla vocacional","Historias para descubrir la llamada de Dios."],
["cultiva-la-fe","Cultiva la fe","Recursos para crecer espiritualmente."]
];
const routes=[
["#/","Inicio"],["#/noticias","Iglesia al día"],["#/programas","Programas"],["#/en-vivo","🔴 En vivo"],
["#/64-datos","64 datos"],["#/diocesis","Diócesis"],["#/editorial","Editorial"],["#/quienes-somos","Quiénes somos"],["#/admin","Admin"]
];
function header(){
 return `<div class="topbar"><div class="wrap"><span>Jóvenes Influencers del Señor · Medio católico de la Diócesis de Ocaña</span><span class="toplinks"><a href="#/quienes-somos">Quiénes somos</a><a href="#/editorial">Editorial</a></span></div></div><header class="channel-nav"><div class="wrap nav"><a class="brand" href="#/"><img src="${C.logo}" alt="Logo Jóvenes Influencers del Señor"><span class="brand-text">Jóvenes Influencers<br><small>del Señor</small></span></a><nav id="menu" class="menu">${routes.map(r=>`<a href="${r[0]}" class="${r[0]==='#/en-vivo'?'live-pill':''}">${r[1]}</a>`).join("")}</nav><button class="hamb" aria-label="Menú" onclick="document.getElementById('menu').classList.toggle('open')">☰</button></div></header>`;
}
function footer(){return `<footer><div class="wrap foot"><div><div class="foot-brand"><img src="${C.logo}" alt="Logo"><div><strong>Jóvenes Influencers del Señor</strong><br><small>Medio de comunicación católico</small></div></div><p>Informamos, formamos y evangelizamos desde la Diócesis de Ocaña.</p><div class="footer-social"><a class="social" href="#" aria-label="Facebook">f</a><a class="social" href="#" aria-label="YouTube">▶</a><a class="social" href="#" aria-label="Instagram">◎</a></div></div><div><strong>Contenido</strong><a href="#/noticias">Iglesia al día</a><a href="#/programas">Programas</a><a href="#/en-vivo">Señal en vivo</a><a href="#/64-datos">64 datos históricos</a></div><div><strong>La Iglesia</strong><a href="#/diocesis">Diócesis de Ocaña</a><a href="#/quienes-somos">Quiénes somos</a><a href="#/editorial">Editorial</a><a href="#/admin">Administración</a></div></div><div class="wrap creator">Sitio web creado por <b>Gamarra TV</b></div><div class="copy">© ${new Date().getFullYear()} Jóvenes Influencers del Señor · Diócesis de Ocaña</div></footer>`}
...
async function home(){
 ...
 try{
  const {createClient}=await import("https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm");
  const s=createClient(C.supabaseUrl,C.supabaseKey);
  const r=await s.from("homepage_activities").select("*").eq("active",true).order("sort_order",{ascending:true}).order("created_at",{ascending:false}).limit(12);
  const rows=r.data||[];
  if(rows.length){
   activitiesHtml=rows.map((x,i)=>`<a class="activity-slide ${i===0?'active':''}" href="${x.link_url||'#'}" ${x.link_url?'':'onclick="return false"'}>
<img src="${x.image_url||''}" alt="${String(x.title||'Actividad').replace(/"/g,'&quot;')}" loading="${i===0?'eager':'lazy'}">
<div class="activity-overlay"><div class="activity-copy">
${x.activity_date?`<div class="activity-date">${x.activity_date}</div>`:''}
<h2>${x.title||''}</h2>
${x.description?`<p>${x.description}</p>`:''}
${x.link_url?`<span class="btn">${x.button_text||'Ver actividad'} →</span>`:''}
</div></div></a>`).join('')+
`<div class="activity-dots">${rows.map((x,i)=>`<button class="activity-dot ${i===0?'active':''}" type="button" data-activity="${i}" aria-label="Actividad ${i+1}"></button>`).join('')}</div>`;
  }else{
   activitiesHtml='<div class="empty" style="padding:50px"><h3>No hay actividades destacadas</h3><p>Agrega actividades desde el panel administrativo.</p></div>';
  }
 }catch(e){
...
<div class="quote-band"><h2>“Comunicar también es una manera de anunciar la Buena Noticia.”</h2><p>Jóvenes Influencers del Señor · Diócesis de Ocaña</p></div>

<section><div class="wrap"><div class="section-top"><div><span class="tag">Conoce nuestro medio</span><h2>Una Iglesia cercana y digital</h2><p>Historia, parroquias, advocaciones, integrantes y contenidos para crecer en la fe.</p></div></div><div class="grid"><a class="card" href="#/diocesis"><h3>⛪ Diócesis de Ocaña</h3><p>Historia, parroquias y advocaciones marianas.</p></a><a class="card" href="#/editorial"><h3>✍️ Editorial</h3><p>Reflexiones y escritos sobre temas importantes.</p></a><a class="card" href="#/quienes-somos"><h3>👥 Quiénes somos</h3><p>Conoce a quienes hacen posible esta misión.</p></a></div></div></section>`}`}
